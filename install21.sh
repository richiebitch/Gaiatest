...
repo_branch="main"
version="0.4.21"
rag_api_server_version="0.13.8"
llama_api_server_version="0.16.9"
...
# Check if WasmEdge is already installed
if [ -d "/root/.wasmedge" ]; then
    warning "    * WasmEdge is already installed at /root/.wasmedge. Skipping installation."
    source /root/.wasmedge/env
    wasmedge_path=$(which wasmedge)
    info "    * Using existing WasmEdge installation at $wasmedge_path."
else
    if [ -n "$ggmlcuda" ]; then
